...
JCSmartFilter.prototype.reload = function(input)
{
	...
	if (this.form)
	{
		...
		$('#modef').show();
		pos = $(input).offset();
		pos2 = $('#smartfilter').offset();

		//change modef position when del and del active obj
		if ( $( '[data-id='+input.id+']' ).length )
		{
			del_obj = $( '[data-id='+input.id+']' );
			if ( $(del_obj).attr('data-clicked')=='y')
			{
				pos = $(del_obj).offset();
				$(del_obj).remove();
			}
		...
		if(pos.top>0)
			$('#modef').css('top', (pos.top-pos2.top-15)+'px');
		else
			$('#modef').css('top', '0px');
			...
			if (arItem.VALUES.hasOwnProperty(i))
			{
				...
				if (!!control)
				{
					...
					if (value.DISABLED)
					{
						...
						$(control).addClass('disabled')/*.attr('disabled',true)*/;
						$(control).parent().addClass('disabled');
						//console.log($(control).closest('ul.drop-panel'));
						//console.log($(control).parents('ul.drop-panel'));
						$(control).closest('ul.drop-panel').append($(control).parent().parent());
					}
					else
					{
						if (label)
		...
		if (!!modef && !!modef_num)
		{
			...
			/*	if (modef.style.display === 'none')
				...
				if(result.SEF_SET_FILTER_URL === ''){
					$('#set_filter').attr('disable','disabled');
				}
				else{
					$('#set_filter').removeAttr('disable');
				}
				//console.log(url,'datafilter');
				//var data = parceAjaxUrl('/');
				//reloadPage(url, data, 'filter_opros');
...
function initRangeSlider()
{
	...
	$('[id ^="c-filt-price-range-"]:not(.noUi-target)').each(function(){
		...
		console.log(catalog_filter_price_slider);
...
$(document).on('change','.index-form__radio[name=section_id]',function(){
	var data = {
		section_id:$(this).val(),
