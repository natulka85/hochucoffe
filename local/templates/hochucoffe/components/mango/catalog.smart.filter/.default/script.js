...
$(document).on('focus','.inp-text',function () {
	console.log('test');
	$(this).attr('current-value', $(this).val());
	$(this).val('');
})

$(document).on('blur','.inp-text',function () {
	if (!$(this).val()) {
...
$(document).on('click','.filter__info-params-value',function(){
	var id = $(this).data('id');
	$('#'+id).click();
})
$(document).on('click','.filter__info-btn',function(){
	if($(this).data('all')==='Y'){
		$('.filter__checkbox:checked').click();
	}
	else{
		var block = $(this).parents('.filter__choose-block');
